...
def load_dicom_volume_as_numpy_from_list(dcmlist):
    ...
    # Make sure that we have correctly constructed the volume from the axial slices!
    hdr = dcmlist[0]
...
def get_predicted_volumes(pred):
    ...
    # Compute the volume of the hippocampal prediction
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    pimg = Image.new("RGB", (1000, 1000))  # creat new image
    draw = ImageDraw.Draw(pimg)

    header_font = ImageFont.truetype("assets/Roboto-Regular.ttf", size=40) #Load a TrueType or OpenType font file, and create a font object.
    ...
    slice_nums = [orig_vol.shape[0]//2, orig_vol.shape[1]//2, orig_vol.shape[2]//3] # centre of each dimension

    # Create the report here and show information that would be relevant to clinicians.
    ...
    pil_orig = orig_img(slice_nums[0])
...
def get_series_for_inference(path):
    ...
    # those PyDicom objects that represent files that belong to the series

    series_for_inference = []
    for i in dicoms:
        if i.SeriesDescription == 'HippoCrop':
            series_for_inference.append(i)
...
if __name__ == "__main__":
    ...
    print(subdirs)
# ['/data/TestVolumes/Study1/36459-FLAIRreg-76244', '/data/TestVolumes/Study1/34909-T1prereg-84610', '/data/TestVolumes/Study1/37916-T2reg-88910', '/data/TestVolumes/Study1/13_HCropVolume', '/data/TestVolumes/Study1/12-T1post-05670']

    # Get the latest directory
    study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)[1] #按时间降序排列
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"section2/out/Result/2020-08-02_1436_Basic_unet/model.pth")
    ...
    # create_report is not complete. Go and complete it.
...
#     os_command("sudo storescp 106 -v -aet HIPPOAI -od ..data/TestVolumes/Study1 --sort-on-study-uid st")
